...
class SwipeView : FrameLayout {
     ...
     */
    var swipeGestureListener: SwipeListener? = null

    /**
     * Animator for the view visible when swiping to the left.
     */
    var leftSwipeAnimator: SwipeAnimator? = null

    /**
     * Animator for the view visible when swiping to the right.
     */
    var rightSwipeAnimator: SwipeAnimator? = null
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        val typedArray = context.obtainStyledAttributes(attrs, R.styleable.SwipeView)
        val swipeLeftRippleColor =
            typedArray.getColorStateList(R.styleable.SwipeView_LeftRippleColor)
        val swipeRightRippleColor =
            typedArray.getColorStateList(R.styleable.SwipeView_RightRippleColor)
        rippleTakesPadding = true
...
//        Log.e(TAG,"activate")
    ...
    companion object {
        ...
        private class PressTimeoutHandler(private val swipeActionView: SwipeView) :
            Handler() {
            var x = 0f
            var y = 0f
            ...
            fun hasOurMessages() = hasMessages(LONG_PRESS) || hasMessages(
                TAP
            )
